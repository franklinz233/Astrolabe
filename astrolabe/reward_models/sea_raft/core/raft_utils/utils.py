...
def bilinear_sampler(img, coords, mode='bilinear', mask=False):
    ...
    img_cont = img.float().contiguous()
    grid_cont = grid.float().contiguous()

    # 针对超大 Batch Size (998,400)，必须禁用 cuDNN
    with torch.backends.cudnn.flags(enabled=False):
        img = F.grid_sample(img_cont, grid_cont, align_corners=True).to(orig_dtype)

    # img = F.grid_sample(img.float(), grid.float(), align_corners=True).to(orig_dtype)
